...
namespace ARPGDemo.BuffSystemSample
{
    ...
    public class BulletState : MonoBehaviour
    {
        ...
        ///单位：秒
        ///</summary>
        public float canHitAfterCreated = 0; //就是一个常见机制所需要的变量
        ...
        /*TODO：这里设置的hp变量表示子弹的可命中次数，感觉也非常不优雅，我估计也可以使用Buff实现*/
        ///<summary>
        ///还能命中几次
        ///</summary>
        public int hp = 1;
        ...
        {
            /*Tip：三种情况，由此及彼、由一般到具体，首先是子弹是否能够发生碰撞（击中），因为这是主体、源头；然后是是否已经击中过传入的目标，其中有一个计时器，
            到点了就会从hitRecords列表中移除对应元素；最后就是看当前传入对象是否处于无敌状态。
            不过对于无敌状态这个，还有不同情况，比如子弹命中它是穿过去吗，还是直接打在它身上消失但是它不会受到伤害呢？*/
            /*Ques：虽然逻辑很清晰，但是对于扩展性的支持感觉不太对劲，可能最后的检测应该放在ChaState的一个公开方法中，这里直接调用其方法，
            因为这本来就属于目标是否能被Hit的逻辑，而在上面的逻辑都属于Bullet能否击中目标的逻辑，就应该用方法把属于各自的逻辑封装起来，然后共同
            完成这里CanHit的逻辑判断。*/
