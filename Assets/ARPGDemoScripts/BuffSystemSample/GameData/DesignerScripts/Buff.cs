...
namespace ARPGDemo.BuffSystemSample.DesignerScripts
{
    /*Tip：注意与DesignerTables命名空间中的Buff区别，这里的Buff类中记录的是BuffModel的回调点的一些预定义好的内容，那里的Buff是BuffModel的内容也就是一个具体的Buff效果的内容，这些内容
    本来应该由策划填表配置的，只是因为这是个演示Buff系统的Demo，没有相关的编辑器脚本。
    这样看起来本来这些回调点的委托也都是预定义的静态数据，那么在编辑器扩展上大概需要序列化字典，然后作为键的字符串就是一个回调效果的名称（唯一性），值就可以自由添加写好的公开方法*/
