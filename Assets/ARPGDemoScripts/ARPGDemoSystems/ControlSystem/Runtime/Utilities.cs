...
namespace ARPGDemo.ControlSystem
{
    ...
    {
        private CinemachineOrbitalFollow freelook; //缩放的目标。
        ...
        public FreeLookZoomer(CinemachineOrbitalFollow _freelook) : this(_freelook, 0.5f, 2.0f)
        {
            // FreeLookZoomer(_freelook, 0.5f, 2.0f);
        }
        //Tip：突然想到重载相对于设置参数默认值的好处，就是要求必须传入这些参数，而不是像默认值那样可以选择性传入，显然是否要使用这种强制性就要看具体的逻辑了。
        public FreeLookZoomer(CinemachineOrbitalFollow _freelook, float _minZoom, float _maxZoom)
        {
            freelook = _freelook;
            topRig = new Vector2(_freelook.Orbits.Top.Height, _freelook.Orbits.Top.Radius);
            middleRig = new Vector2(_freelook.Orbits.Center.Height, _freelook.Orbits.Center.Radius);
            bottomRig = new Vector2(_freelook.Orbits.Bottom.Height, _freelook.Orbits.Bottom.Radius);
        ...
        private void SetRig()
        {//这就是以绝对数值的方式来计算。
            freelook.Orbits.Top.Height = topRig[0] * zoomCounter;
            freelook.Orbits.Top.Radius = topRig[1] * zoomCounter;
            freelook.Orbits.Center.Height = middleRig[0] * zoomCounter;
            freelook.Orbits.Center.Radius = middleRig[1] * zoomCounter;
            freelook.Orbits.Bottom.Height = bottomRig[0] * zoomCounter;
            freelook.Orbits.Bottom.Radius = bottomRig[1] * zoomCounter;
        }
