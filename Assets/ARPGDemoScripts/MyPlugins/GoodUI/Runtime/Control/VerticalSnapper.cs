...
namespace MyPlugins.GoodUI
{
    ...
    [AddComponentMenu("GoodUI/Controls/VerticalSnapper")]
    public class VerticalSnapper : UIBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler
    {
        public float duration = 0.2f;
        public int viewCount = 0;
        public float itemSize = 0f; //因为这里只用于竖直情况，所以此处指的是元素的高度
        ...
        private Tween snapTween; //保存用于吸附的Tween，只是为了可以使用Kill方法取消。
        ...
        protected override void Start()
        {
            ...
            // viewCount = (int)(scrollRect.viewport.rect.height / itemSize);
            itemSize = items[0].rect.height;
            viewCount = Mathf.RoundToInt(viewport.rect.height / itemSize);

            maxY = (items.Length - viewCount) * itemSize;
            // content.a
        ...
        */
        ...
        private void Snap(int dir)
        {
            ...
            int upIndex = Mathf.Clamp((int)(content.anchoredPosition.y / itemSize), 0, items.Length - 1);
            ...
            // int index = (int)(content.anchoredPosition.y / itemSize); //这里就不能用RoundToInt了，就是用int转换丢弃小数部分
            //处于边界情况，具体来说是已经完全接近于目标情况，那么就直接设置，也就是“修正”，并且不用也一定不要执行后续逻辑，要不然往往都会出一些bug。
            // if (index * itemSize - content.anchoredPosition.y <= 0.05f)
            // {
            //     content.anchoredPosition = new Vector2(0f, index * itemSize);
            //     return;
            // }
            float lowerPosition = upIndex * itemSize;
            float upperPosition = Mathf.Clamp(upIndex + 1, 0, items.Length - viewCount) * itemSize; //实际能超出边界的应该除去可视部分
            ...
            snapSequence.Join(content.DOAnchorPosY(targetPosition, duration, true));
        }
        ...
        private bool EdgeCorrect()
        {
            ...
            if (Mathf.Approximately(content.anchoredPosition.y, maxY))
            {
            ...
            else if (Mathf.Approximately(content.anchoredPosition.y, 0f))
            {
        ...
        public void OnBeginDrag(PointerEventData eventData)
        {
            ...
            // int index = Mathf.RoundToInt(content.anchoredPosition.y / itemSize);
        ...
        public void OnDrag(PointerEventData eventData)
        {
            ...
            if (eventData.delta.y > 0) dragDir = 1;
            else if (eventData.delta.y < 0) dragDir = -1;
        ...
        {
            // if (content.anchoredPosition.y - content.anchoredPosition.y / itemSize <= 0.05f)
            // {
            //     content.anchoredPosition = new Vector2(0, Mathf.RoundToInt(content.anchoredPosition.y / itemSize) * itemSize);
            //     return;
            // }

            // int upIndex = (int)(content.anchoredPosition.y / itemSize);
            ...
            float upItemOffset = content.anchoredPosition.y - upIndex * itemSize;
            float downItemOffset = (content.anchoredPosition.y + viewport.rect.height) - downIndex * itemSize;
