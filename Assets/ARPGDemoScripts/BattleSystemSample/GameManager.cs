...
namespace ARPGDemo.BuffSystemSample
{
    public class GameManager : MonoBehaviour
    {
        ...
        void Start()
        {
            //代码查找指定对象，通常是会在编辑时利用序列化提前设置，运行时在代码中检查如果没有的话就尝试查找。
            ...
            //学习技能。参数SkillModel是提供基础数据（静态数据），在LearnSkill中会将其封装为SkillObj
        ...
        {
            //管理一下视觉特效，看哪些需要清除了
            //Ques：这里不知道为何不在foreach循环中判断Value为空时直接使用Remove移除，因为看起来并没有必要先找出要移除的键再统一移除。
        ...
        //根据prefab下的资源创建东西。相当于对Instantiate方法的封装。
        ...
        public GameObject CreateCharacter(string prefab, int side, Vector3 pos, ChaProperty baseProp, float degree, string unitAnimInfo = "Default_Gunner", string[] tags = null)
        {
            ...
            //注意看Assets/Resources/Prefabs/Character/CharacterObj.prefab该预制体上已经把各个组件都挂载好了，直接实例化即可。
            ChaState cs = chaObj.GetComponent<ChaState>();
            // Debug.Log($"ChaState: {cs}");
            if (cs)
            {
