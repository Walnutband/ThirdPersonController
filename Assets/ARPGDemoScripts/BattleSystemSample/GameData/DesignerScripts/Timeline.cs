    ...
    public class Timeline{
        ...
        private static void CreateAoE(TimelineObj tlo, params object[] args){
            ...
            if (tlo.caster){
                UnitBindManager ubm = tlo.caster.GetComponent<UnitBindManager>();
                if (!ubm) return;
                //从参数args传入AoeLauncher实例，克隆就是根据相同数据再创建一个新的实例，然后在此处更新一下相关数据，然后调用SceneVariants.CreateAoE方法传入AorLauncher即可创建AOE
        ...
        ///<summary>
        ///给timeline的caster添加一个buff（释放某技能的同时获得一个什么加成，这是常见机制）
