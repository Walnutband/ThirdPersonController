...
namespace ARPGDemo.BuffSystemSample
{
    ...
    public class DamageInfo
    {
        ...
        ///</summary>
        public Damage damage; //因为在数值、在逻辑上伤害和治疗和对立统一的，只是大多数情况下是表现为伤害。
        ...
        /*Tip：可以想象，在程序层面，应该把这些基本机制全部考虑到，简单来说就是把变量设置出来，甚至把相关的方法都应该一并写好，然后才是根据策划的设计来决定要使用哪些变量、调用哪些方法，
        这样的话游戏才能在最底层的程序层面为创新的游戏设计留下开发空间，而且并不会有什么代价（额外占用的内存和计算量具体来说完全可以忽略不计），除了程序员会加大工作量以外。*/
        ///<summary>
        ///是否命中，是否命中与是否暴击并不直接相关，都是单独的算法
        ///作为一个射击游戏，子弹命中敌人是一种技巧，所以在这里设计命中了还会miss是愚蠢的（就是因为需要操作，而Miss往往会出现在回合制游戏中，因为那不需要操作技巧，只需要点击）
        ...
        /*Tip：属于基本机制 ，比如属性伤害攻击后积累异常状态量表，最典型的火伤造成一段持久的灼烧伤害，立刻的伤害就存储在Damage中，而延后的伤害以及一些特殊效果就存储在这里的AddBuffInfo中*/
        ///<summary>
        ///伤害过后，给角色添加的buff（属于基本机制 ，比如属性伤害攻击后积累异常状态量表，最典型的火伤造成一段持久的灼烧伤害，立刻的伤害就存储在Damage中，而延后的伤害以及一些特殊效果就存储在这里的AddBuffInfo中）
        ///</summary>
    ...
    /*Tip：该结构体就是记录的原始的伤害数值，因为有多种伤害会影响到执行逻辑，所以将其分为各自的变量，封装到该结构体中。*/
    ///<summary>
    ...
    /*TODO：这里的枚举类型应当以二进制来表示，直接原因就是DamageInfo中会有一个DamageInfoTag[]数组而不是单个的DamageInfoTag，使用二进制就可以直接通过位运算来判断，而且就只需要
    存储单个的DamageInfoTag，因为通过比特位囊括了所拥有的各个Tag的信息，否则的话就像上面的requireDoHurt、isHeal等方法一样需要遍历数组来逐个判断，这在运行效率上是O(1)与O(n)的区别，
    显然值得重视。*/
    ///<summary>
    ...
    { //枚举类型，具有明确含义，且可以直接在编辑器中指定成员。
        directDamage = 0,   //直接伤害
        periodDamage = 1,   //间歇性伤害（持续伤害？）
