...
namespace ARPGDemo.BuffSystemSample
{
    ...
    public struct BulletModel
    {
        ...
        )
        {
            this.id = id;
            this.prefab = prefab;
            /*在DesignerScripts.Bullet类中存储了一系列静态字典成员，存储了各个委托的预定义的一些回调方法，通过字符串作为键来访问，所以传入的字符串参数本质上是回调方法的名称，
            然后在这里的构造函数中就可以直接指定作为回调点的委托所要执行的逻辑，而且根据传入的字符串可以在不同实例中都有所变化。*/
    ...
    public class BulletHitRecord
    {
        ...
        /*Tip：这是个很关键的变量，而对于它的作用来说，可能这一个变量还不够。也就是攻击判定的问题，无法仅仅依赖物理系统的判定来实现各种甚至是很常见的功能，
        至少都要用到计时器，而且还要做判定、做排除*/
        ///<summary>
        ///多久之后还能再次命中，单位秒
        ///</summary>
