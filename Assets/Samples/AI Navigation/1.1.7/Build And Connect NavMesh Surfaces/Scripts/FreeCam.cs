    ...
    public class FreeCam : MonoBehaviour
    {
        ...
        void Update()
        {
            if (axes == RotationAxes.MouseXAndY)
            {
                ...
                float rotationX = transform.localEulerAngles.y + Mouse.current.delta.x.ReadValue() * sensitivityX;
            ...
            {
                transform.Rotate(0, Mouse.current.delta.x.ReadValue() * sensitivityX, 0);
            }
            else
            {
                rotationY += Mouse.current.delta.y.ReadValue() * sensitivityY;
            ...
            var xAxisValue = Keyboard.current.dKey.ReadValue() - Keyboard.current.aKey.ReadValue();
            // var zAxisValue = Input.GetAxis("Vertical");
            var zAxisValue = Keyboard.current.wKey.ReadValue() - Keyboard.current.sKey.ReadValue();
            if (lockHeight)
            {
