    ...
    public class MeshTool : MonoBehaviour
    {
        ...
        void Update()
        {
            ...
            if (Physics.Raycast(ray.origin, ray.direction, out m_HitInfo))
            {
                ...
                if (Mouse.current.leftButton.wasPressedThisFrame || (Keyboard.current.spaceKey.isPressed && !Keyboard.current.leftShiftKey.isPressed))
                {
                ...
                else if (Mouse.current.rightButton.wasPressedThisFrame || (Keyboard.current.spaceKey.isPressed && Keyboard.current.leftShiftKey.isPressed))
                {
                    ModifyMesh(-m_Power * displacement, m_HitInfo.point);
                    if(m_Surface != null)
                    {
                ...
                else if(Mouse.current.leftButton.wasPressedThisFrame || Mouse.current.rightButton.wasPressedThisFrame || Keyboard.current.spaceKey.wasReleasedThisFrame)
                {
                    if (m_Surface != null)
                    {
                        if (!m_LastNavMeshUpdate.isDone)
