...
/*Tip：这里应该指的是一个组合绑定，它的各个部分有可能定义在不同的控制方案（就是设备类型）中，但是在实际游戏中似乎没有这样的，比如经典的WASD，这种操作通常不允许重绑定按键，
就是说，这种似乎没什么应用价值，大概在更具有创新交互的游戏中才可能有所应用？？*/
...
namespace UnityEngine.InputSystem.Samples.RebindUI
{
    ...
    public class RebindActionUI : MonoBehaviour
    {
        ...
        public InputActionReference actionReference
        {
            ...
            {
                m_Action = value;
                UpdateActionLabel(); //更新左边的动作文本，表示这是在设置哪个操作。
                UpdateBindingDisplay(); //更新当前绑定的按键的显示文本
            }
        }
        ...
        /// according to
        /// <para></para>
        /// 获取当前所引用的InputAction以及所绑定的按键在该InputAction的Binding数组中的索引
        /// </summary>
        ...
        public bool ResolveActionAndBinding(out InputAction action, out int bindingIndex)
        {
            ...
            var bindingId = new Guid(m_BindingId); //因为InputBinding按照Guid来比较
            bindingIndex = action.bindings.IndexOf(x => x.id == bindingId); //在当前所引用的InputAction中所具有的Binding中的第几个。
            if (bindingIndex == -1)
            {
        ...
        {
            //Tip：找到当前绑定按键的索引，随后将重新绑定的按键放到该位置上。
            if (!ResolveActionAndBinding(out var action, out var bindingIndex))
                return;

            // If the binding is a composite, we need to rebind each part in turn.
            if (action.bindings[bindingIndex].isComposite)
            {//Composite Binding本身也属于一个Binding，然后索引是接着它的Part Binding，而不是下一个非Part Binding
        ...
        private void PerformInteractiveRebind(InputAction action, int bindingIndex, bool allCompositeParts = false)
        {
            ...
            void CleanUp()
            {
                ...
                action.Enable(); //（重新）启用当前引用的InputAction
            }

            //Fixes the "InvalidOperationException: Cannot rebind action x while it is enabled" error
            action.Disable();    //不能在启用状态下重绑定。

            // Configure the rebind. 配置RebindOperation
            m_RebindOperation = action.PerformInteractiveRebinding(bindingIndex)
                .OnCancel(
                    operation =>
                    {
                        m_RebindStopEvent?.Invoke(this, operation);
                        if (m_RebindOverlay != null) //使用一个UI界面来提示当前是否正在重绑定，并且通常使用文本来显示正在绑定什么按键，其实重绑定操作本身与UI无关。
                ...
                .OnComplete(
                    operation =>
                    {
                        ...
                        // for the next part.
                        //Tip：对于Composite Binding的实现非常朴素，就是递归而已
            ...
            {
                var text = !string.IsNullOrEmpty(m_RebindOperation.expectedControlType) //指定了InputAction即可。
            ...
            m_RebindStartEvent?.Invoke(this, m_RebindOperation); //自定义回调，在重绑定开始时

            m_RebindOperation.Start(); //开始执行重绑定操作。
        }
