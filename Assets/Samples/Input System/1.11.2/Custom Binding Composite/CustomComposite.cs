...
public class CustomComposite : InputBindingComposite<Vector2> //反射获取，该类名就是添加Binding时的菜单项名。
{
    ...
    [InputControl(layout = "Axis")] //表示作为一个Part，并且layout就是Contorl Type，直接决定能够绑定设备上的哪些控件。
    ...
    //         .With("Multiplier", "<Gamepad>/rightTrigger")
    //         .With("Stick", "<Gamepad>/leftStick");
    /*Ques：看注释说可以在Action Editor中编辑，但是测试发现并没有显示，而是在InputAction作为字段被序列化时，在其检视器中可以显示，并且图形化编辑。*/
