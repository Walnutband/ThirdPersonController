...
namespace MyPlugins.GoodUI
{
    ...
    public class UIConfig
    {
        ...
        public UIViewType uiType; //具体来说，UIType中定义的各个常量就是UIView的各个派生类的名称
        //UI层级。还是根据具体项目需求，通过划分层级可以更好地处理不同UI视图之间的遮挡关系、打开和关闭，等等功能，以及在UI控件的组织上都更加有条理、有逻辑。
        public UILayer uiLayer;
        public Type viewType; //这个Type指的是挂载在预制体根对象上的逻辑组件的类型，而上面的UIViewType指的是
            ...
            return ResourceManager.Instance.LoadAssetAsync<UnityEngine.TextAsset>(UIConfigPath, (textAsset) => //这个回调是加载结束时触发,传入加载得到的TextAsset实例。
            {
                if (textAsset != null)
                {
                    ...
                    foreach (var config in uiConfigs)
                    {
                        ...
                        Type viewType = GetType(config.UIViewType.ToString()); //GetType获取的类型名就会包含命名空间，可以通过Type.Name获取类型名，Type.FullName获取完整名。
                        if (viewType == null) //前后的区别在于，这里会加上UIConfig所在的命名空间名称。但是我比较疑惑如果前面都没有找到，那后面加上命名空间就有可能找到吗？
                        {//命名空间.类型名
                            viewType = GetType($"{typeof(UIConfig).Namespace}.{config.UIViewType}");
                        }
