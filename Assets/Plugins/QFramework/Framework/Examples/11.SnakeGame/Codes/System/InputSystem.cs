...
namespace SnakeGame
{
    ...
    public class InputSystem : AbstractSystem, IInputSystem
    {
        ...
        /// 用于表示输入状态
        /// </summary>
        private enum E_InputState { Down, Up, Hold, Null } //按下、抬起、按住、无输入
        //按键集成。就是将相同输入的按键放在同一个数组中，显然完全是按照键盘设置的
        ...
        /// <summary>
        /// 更新函数（获取方向输入并发送事件DirInputEvent）
        /// </summary>
        private void OnUpdate()
        {
            CheckMoveDir(BindableInput(mLeftKeys), BindableInput(mRightKeys), ref mDirInput.hor); //检测水平输入
            CheckMoveDir(BindableInput(mDownKeys), BindableInput(mUpKeys), ref mDirInput.ver); //检测竖直输入
        ...
        {//注意贪吃蛇的方向输入是触发性输入，而不是持续性输入，所以在此专门处理了后两种情况，这样改变方向的操作就更连贯了
