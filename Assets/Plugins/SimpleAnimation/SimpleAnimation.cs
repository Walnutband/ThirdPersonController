...
public partial class SimpleAnimation: MonoBehaviour
{
    public interface State
    {
        ...
        bool isValid { get; }
        float time { get; set; } //绝对时刻，对应于duration
        float normalizedTime { get; set; } //归一化时刻，对应进度
        float speed { get; set; } //播放速度
        string name { get; set; } //标识符，由此访问
        float weight { get; set; } //权重
        float length { get; } //总时长，就是duration本身
        AnimationClip clip { get; } //引用的动画片段，这才是源数据
        WrapMode wrapMode { get; set; } //所处时刻在超出Clip关键帧范围之后应该如何处理
    ...
    //注意理解m_Playable即自定义的SimpleAnimationPlayable的作用，
