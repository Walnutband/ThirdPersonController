...
public partial class SimpleAnimationPlayable : PlayableBehaviour
{
    ...
    public override void OnPlayableCreate(Playable playable)
    {
        ...
        //第三个参数已经过时，标记是否使用归一化权重。现在会直接使用输入的权重进行计算，至于要不要归一化，就完全看自定义的逻辑。
        ...
        graph.Connect(m_Mixer, 0, self, 0); //将Mixer的0号输出端口连接到该ScriptPlayable的0号输入端口。
    }
    ...
    private void UpdateDoneStatus()
    {
        if (!m_States.AnyStatePlaying())
        {
            ...
            {
                Debug.Log($"调用UpdateDoneStatus，且触发onDone事件");
                onDone();
            }
        }
