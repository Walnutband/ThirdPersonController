...
namespace RPGCore.Dialogue.Editor
{
	public class DialogueEditorWindow : EditorWindow
	{
		...
		//private
		[OnOpenAsset(1)] /*Tip：这里Unity内置的一个特性，会在双击资产文件时调用所标记的方法，所以能够用来双击自定义的资产文件时打开自定义的编辑窗口*/
		public static bool OpenWindowWithAsset(int instanceId, int line)
		{
			//Unity内部记录了运行时的实例ID与其实例的映射关系。
		...
		private void OnEnable()
		{
			...
			//加载GraphView
			/*Tip：GraphView的UI结构完全是运行时定义的，所以不需要Uxml文件。直接构造实例，添加到已有的VisualElement中即可。*/
		...
		public void UpdateNodeInspector(DgNodeBase nodeData)
		{
			...
			while (nodeProperty.NextVisible(false))
			{
				...
				//绑定路径确实正好对应propertyPath。设置了绑定路径之后就会自动根据其SerializedProperty创建PropertyField对象
				inspectorItem.Field.bindingPath = nodeProperty.propertyPath; 
				//意思是不能在检视器中修改type字段，其他字段随意。
