    ...
    public class CardStack2D : MonoBehaviour
    {
        ...
        void Update()
        {
            if (canUseHorizontalAxis)
            {
                ...
                if ((GoodUIInputManager.GetAxisRaw("Horizontal") < 0 || GoodUIInputManager.GetKey(leftButton)) && cardArrayOffset > 0)
                {
                    cardArrayOffset--;
                    StartCoroutine(ButtonCooldown());
                }
                else if ((GoodUIInputManager.GetAxisRaw("Horizontal") > 0 || GoodUIInputManager.GetKey(rightButton)) && cardArrayOffset < cards.Length - 1)
                {
                    cardArrayOffset++;
                    StartCoroutine(ButtonCooldown());
                }
