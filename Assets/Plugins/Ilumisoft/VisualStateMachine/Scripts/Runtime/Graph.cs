namespace Ilumisoft.VisualStateMachine
{
    ...
    //字段必须是可序列化的： 类中的所有字段必须是可序列化的。如果有不可序列化的字段，可以使用 [NonSerialized] 特性来标记该字段，确保其在序列化过程中被忽略。
    //事件和委托不能序列化： 事件和委托不能被序列化。如果类中包含事件或委托，必须使用 [NonSerialized] 特性来忽略它们。
    [System.Serializable]
    public class Graph : ISerializationCallbackReceiver
    {
        ...
        /// <summary>
        /// Tries to get the node with the given id.尝试根据给定的id获取节点
        /// </summary>
        /// <param name="id"></param>
