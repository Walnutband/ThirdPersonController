...
namespace CrashKonijn.Goap.Resolver
{
    public class GraphResolver : IGraphResolver
    {
        ...
        {
            //Tip：返回的Graph就是已经构建好的图了，包含了图中所有的节点数据，而节点自己存储了所连接的节点信息，每一个根节点都是一棵树。
            this.graph = new GraphBuilder(keyResolver).Build(actions);
            //以下数据都来源于上面创建的Graph。
            this.indexList = this.graph.AllNodes.ToList();
            this.actionIndexList = this.indexList.Select(x => x.Action).ToList(); //Action只有原始数据，没有连接关系。
            //SelectMany展开嵌套集合，Select转换每个元素。
        ...
        {//TODO：使用Collections优化，还需要进一步学习相关技术。
#if UNITY_COLLECTIONS_2_1
