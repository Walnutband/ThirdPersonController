...
namespace UnityHFSM
{
		...
		IActionable<TEvent>
	{
		...
		private struct PendingTransition
		{
			...
			//因为这是结构体，不可为空（虽然有相关的语法糖，但是终究只是表面），所以需要这里额外的一个字段表达与空相同的作用。
			public bool isPending;

			//由于默认的互补关系，所以使用一个isExit就可以表达是Enter还是Exit的Transition
		...
		private void ChangeState(TStateId name, ITransitionListener listener = null)
		{
			...
			//在过渡完全完成之后（监听方法也已经执行），通知状态机发生了状态变化。
		...
		/// 由于可分层，所以一个子状态机中的状态退出并转换到其他状态机中的状态时，就需要通知所在的子状态机也需要执行相关的转换逻辑。
		/// </summary>
