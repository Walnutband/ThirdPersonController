...
namespace Animancer
{
        ...
        IAnimationClipCollection
    {
        ...
        {
            /*Tip：惰性初始化，突然想到这种属性的getter，隐藏的含义就是在访问它时获取它时必然能够获取到非空的实例，典型的就是初始化，而
            对应的，setter典型就是触发值改变事件，总之就是强调一个流程，强调正确的流水线工作。*/
            get
            {
                InitializeGraph();
                return _Graph;
        ...
        //Ques：以下这样直接获取Graph的成员就是因为一个AnimancerComponent与一个AnimancerGraph完全绑定，按理来说可以切换，但是一般不会这样做。但是总感觉这种写法不太好呢？？
        ...
        //隐式转换的重载，也就是可以直接将AnimancerComponent实例赋值给AnimancerGraph或AnimancerLayer类型的引用
        ...
        /// </summary>
        protected virtual void Reset() //设置为protected而不是直接private，纯粹是留下扩展性，通过继承而重写方法来实现扩展。
        {
            OnDestroy();
        ...
        {
            /*Ques：没看懂这里为何不直接访问属性Graph，而不是这里的字段_Graph？因为属性Graph中就会检查初始化之类的逻辑，为何在这里特意设置分支呢？
            不过也可以想到一个合理的理由，因为OnEnable并不处理初始化逻辑（初学者可能误以为它处理初始化逻辑，但其实Awake和Start才是处理初始化逻辑），
            而是处理启用逻辑，与禁用状态相对，也就是该实例在禁用状态和启用状态会发生哪些变化、存在哪些差异，这就是OnEnable和OnDisable要处理的逻辑，
            所以这里就是如果有Graph，那么就恢复（如果暂停了的话），没有就不执行任何逻辑，不需要也不应该涉及到Graph的初始化逻辑。*/
            if (IsGraphInitialized)
            {
                _Graph.UnpauseGraph();
        ...
        public void InitializeGraph()
        {
            ...
            //设置PlayableGraph名称，游戏对象名+(Animancer)
            AnimancerGraph.SetNextGraphName(name + " (Animancer)");
            //Tip：初始化时，如果有就直接赋值，如果没有的话，后续也会进行（惰性）初始化。
            _Graph = new(_Transitions?.Library); //生成AnimancerGraph
            _Graph.CreateOutput(_Animator, this); //创建层级混合节点，并且立刻开始播放Graph。
            OnInitializeGraph();
        }
        ...
        //在AnimancerComponent所在游戏对象上获取Animator组件

        /************************************************************************************************************************/

        //TODO：验证Graph的初始化，仅用于编辑时，牵涉到过多逻辑，暂时不思考。
#if UNITY_ASSERTIONS
        /// <summary>[Assert-Only]
        ...
        /// <para>使用索引器而不是GetLayer，大概就揭示出了Play的性质，播放指定动画，同时取消其他动画。</para>
        /// <para>可见都是转换为AnimancerState处理的，因为这才是在该系统中同层的类型</para>
        /// </remarks>
        public AnimancerState Play(AnimationClip clip) => Graph.Layers[0].Play(States.GetOrCreate(clip)); 
        ...
        //播放过渡Transition（ClipTransition和TransitionAsset）
        /// <summary>
        ...
        public AnimancerState TryPlay(object key) => Graph.Layers[0].TryPlay(key);
