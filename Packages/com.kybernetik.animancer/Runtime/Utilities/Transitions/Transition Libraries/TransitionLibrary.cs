...
namespace Animancer.TransitionLibraries
{
        ...
        ICopyable<TransitionLibrary>
    {
        ...
        //获取在Transition Library编辑窗口中编辑的FadeDuration过渡时间
        /// <summary>[Pro-Only]
        ...
        /*Tip：这里的Play方法被AnimancerLayer的Play方法调用，然后又调用AnimancerLayer中的Play方法，这一个来回纯粹是为了调用TransitionLibrary中的GetFadeDuration方法。
        这就是所谓的TransitionLibrary的覆盖功能，就是在Transition Library编辑窗口中编辑的过渡时间*/
        //TODO；不过这是很显然的耦合，不如直接在AnimancerLayer中调用TransitionLibrary的某公开方法获取FadeDuration，始终是在AnimancerLayer内部调用Play方法，而不是由这里的TransitionLibrary调用而跳转回到AnimancerLayer
        /// <summary>
        /// Calls <see cref="AnimancerLayer.Play(ITransition, float, FadeMode)"/>
        /// with the fade duration potentially modified by this library.
        /// </summary>
        public AnimancerState Play(AnimancerLayer layer, ITransition transition)
            => layer.Play(transition, GetFadeDuration(layer, transition), transition.FadeMode);
