...
namespace Animancer
{
        ...
        IHasDescription
    {
        ...
        //Tip：从FadeIn和FadeOut的类型可见，可以从多个节点渐出，但只能渐入一个节点。
        ...
        // AnimancerLayer通过该方法实现对其子节点（AnimationClipPlayable）的过渡（注册）
        ...
        public void SetFadeIn(AnimancerNode fadeIn)
        {
            ...
            FadeIn = new(fadeIn); //通过AnimancerNode组合（封装）构建一个专门用于过渡的NodeWeight实例
            fadeIn.FadeGroup = this; //所在的过渡组。
        }
        ...
        public void StartFade()
        {
            //注册进入_PreUpdatables容器，参与每帧更新
            Graph?.RequirePreUpdate(this);

            //OnStartFade就是一个回调方法，渐入节点和渐出节点都会在开始过渡时触发。
        ...
        /// <inheritdoc/>
        /// <see cref="IUpdatable.List.UpdateAll">
        /// <summary>实现过渡效果（更新渐入节点和渐出节点的权重）</summary>
        public override void Update()
        {
            if (!IsValid)
            {//有两层含义，一方面是不会执行这一帧本来应该执行的更新逻辑，一方面是之后再也不会调用自身的该Update方法（因为从容器中移除了）
                Cancel();
                return;
            }
            ...
            //Tip：实际经过的时间，经过父节点的速度和自身的速度共同的缩放之后，才是NormalizedTime真正的变化量。
        ...
        private void Finish()
        {
            NormalizedTime = 1; //经典的浮点修正

            //参考AnimancerGraph中的KeepChildrenConnected（实际在AnimancerNodeBase中就有所定义了）
        ...
        //为属于当前过渡过程（FadeGroup）中的节点设置指定权重
        private void ApplyWeight(AnimancerNode node, float weight)
        {
            //Tip：对于封装，有一个基本问题是，如何保证封装类的某变量与所封装对象的对应变量的值保持同步。这里很显然对于Playable节点的输入权重和AnimancerNode自身记录的权重是手动同步的。
            node._Weight = weight;
        ...
        internal void ChangeGraph(AnimancerGraph graph)
        {
            if (Graph == graph)
                return;
            //Tip：已经脱离了FadeGroup的child所在的Graph，如果与该FadeGroup所在的Graph不同的话，就
            Graph?.CancelPreUpdate(this);
            Graph = graph;
            Graph?.RequirePreUpdate(this);
