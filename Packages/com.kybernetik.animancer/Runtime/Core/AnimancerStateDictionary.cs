...
namespace Animancer
{
        ...
        IEnumerable<AnimancerState>
    {
        ...
        /// <summary>The <see cref="AnimancerGraph"/> at the root of the graph.
        /// <para>状态表的所属Graph</para>
        /// </summary>
        ...
        {
            //Ques：没看懂这里为何不直接使用TryGetValue方法呢？照样可以通过返回值判断是否获取到了对应值。
            if (key == null)
            {
                state = null;
                return false;
        ...
        public AnimancerState GetOrCreate(AnimationClip clip, bool allowSetClip = false)
            => GetOrCreate(Graph.GetKey(clip), clip, allowSetClip); //传入键和内容（Clip）
        ...
        public AnimancerState GetOrCreate(object key, AnimationClip clip, bool allowSetClip = false)
        {
            ...
            {//已经存在记录的状态
                ...
                if (!ReferenceEquals(state.Clip, clip)) //是否引用的同一个Clip（显然是通过.Net框架中记录的类似元数据来实现的判断）
                {
                    if (allowSetClip)
                    {//从此可见，allowSetClip其实是是否要更新（改变）已存在状态对应的Clip
                        state.Clip = clip;
                    }
                    else
                    {
