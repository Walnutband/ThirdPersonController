...
namespace Animancer
{
    ...
    partial struct AnimancerEvent
    {
        ...
        public partial class Sequence :
            IEnumerable<AnimancerEvent>, //可迭代AnimancerEvent实例
            ICloneable<Sequence>
        {
            /************************************************************************************************************************/
            ...
            /// <summary>All of the events in this sequence, excluding the <see cref="EndEvent"/>.
            /// <para>存储在该序列中的所有事件（不包括EndEvent）</para>
            /// </summary>
            ...
            //存储事件索引与其标识符StringReference的映射关系，由于数组下标就具有索引的功能，所以此时可以等同于字典中的键。
            ...
            /// <summary>Returns the index of the first event with the specified `name`.
            /// <para>返回第一个带有指定name的事件的索引</para>
            /// </summary>
            ...
            public Sequence()
            {
                _Events = Array.Empty<AnimancerEvent>(); //空数组，并非空引用。
            }
            ...
            public void SetCallback(int index, Action callback)
            {
                ...
                ref var animancerEvent = ref _Events[index];
                //TODO：这里直接等于，似乎不妥，因为传入的往往是一个方法，那么应该是+=添加到Action委托中，而不是直接覆盖。其实RemoveCallback都是用的-=，不知道这里为何没有使用+=
                animancerEvent.callback = callback;
                Version++;
            }
