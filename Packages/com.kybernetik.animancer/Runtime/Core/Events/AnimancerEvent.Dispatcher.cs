...
namespace Animancer
{
    ...
    partial struct AnimancerEvent
    {
        ...
        public class Dispatcher : IHasDescription
        {
            ...
            /// <summary>The target state.
            /// <para>事件要分发给的状态对象</para>
            /// </summary>
            ...
            public Dispatcher(AnimancerState state)
            {
                ...
                State = state;
                _PreviousNormalizedTime = state.NormalizedTime; //State此时的归一化时间
            ...
            {
                //length是该State的总长度，normalizedTime是当前时刻占总长度的比例（即归一化时刻），isLooping其实是该State对应的AnimationClip是否循环
                ...
                if (length > 0)
                {
                    ...
                    //Tip：将以EndEvent为基本内容的封装对象Invocation添加到AnimancerEvent的InvocationQueue队列中。
            ...
            //触发GeneralEvents常规事件（就是非End结束事件）
