...
namespace Animancer
{
    ...
    public partial struct AnimancerEvent : IEquatable<AnimancerEvent>
    {
        ...
        /// <summary>The delegate to invoke when the <see cref="normalizedTime"/> passes.
        /// <para>以归一化时间作为参考，超过该时间就触发该事件，也就是调用注册的回调方法</para>
        /// </summary>
        ...
        /// <summary>Creates a new <see cref="AnimancerEvent"/>.
        /// <para>传入触发时刻（归一化时间）以及Action</para>
        /// </summary>
        ...
        public readonly void DelayInvoke(
            ...
            => Invoker.Add(new(this, eventName, state)); //封装成为Invocation实例放入到InvocationQueue队列中。
