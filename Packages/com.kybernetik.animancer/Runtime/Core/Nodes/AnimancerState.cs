...
namespace Animancer
{
        ...
        ICopyable<AnimancerState>
    {
        ...
        /// <summary>Sets the <see cref="AnimancerNodeBase.Graph"/>.
        /// <para>设置State所在的Graph，以及创建对应的Playable并连接到指定端口（这才是落实了）</para>
        /// </summary>
        ...
        {
            //总之在同一Graph内部变化的话，只会执行极少数的逻辑，一旦跨越了Graph，就会额外执行大量逻辑。
            if (Graph == graph)
                return;
            //显然AnimancerState是可以迁移所在的Graph
        ...
        //能进入该方法，就说明新Graph与旧Graph（当前Graph）不是同一个。
        private void RemoveFromOldGraph(AnimancerGraph newGraph)
        {
            ...
            //Ques：按理来说能进入该方法，就说明新Graph与旧Graph（当前Graph）不是同一个，难道这里的Parent还可能与该节点不在同一个Graph中吗？
            if (Parent != null && Parent.Graph != newGraph)
            {
            ...
            _Time = TimeD;
            /*Tip：从这里可以看到AnimancerState的一个封装效果，封装了Playable节点，具备了在不同Graph之间迁移的能力*/
            DestroyPlayable();
        }

        //Tip：该方法的逻辑从功能上看是将AnimancerState添加到新的AnimancerGraph，并不要求先存在Playable，所以在以下的逻辑中就会CreatePlayable，即保证底层是合理的，从而保证上层的稳定运行。
        private void AddToNewGraph()
        {
            if (Graph != null)
            {
                Graph.States.Register(this); //注册到新Graph的状态字典中。
                //从封装器AnimancerState落实到Playable
                CreatePlayable();
            }
            //意思是将该节点作为根节点的一整个节点树都移动到新Graph中。
        ...
        public void SetParent(AnimancerNode parent)
        {
            ...
            if (parent == null)
            {
                ...
                Index = -1; //没有parent了，因为index就是记录的连接到parent的输入端口号
                return;
            }
            //Ques：可见State可以改变所在的Graph以及所连接的节点（甚至可以被共享？），
            //Tip：含义是，改变Parent之后，就要进入到Parent所在的Graph，连接到Graph。
            SetGraph(parent.Graph); //创建Playable节点
            Parent = parent;
            parent.OnAddChild(this); //会连接节点
            CopyIKFlags(parent);
            FadeGroup?.ChangeParent(this);
        }
        ...
        //Ques：像这种变量，现在也想不通使用internal声明字段，还是使用字段+属性的方式好。不过大概可以认为internal只会出现在将会自成一个系统的代码中。
        ...
        {
            if (_IsPlaying == isPlaying) //状态不变
                return;

            _IsPlaying = isPlaying;

            if (_Playable.IsValid())
            {//落实到Playable的Play和Pause
                if (_IsPlaying)
                    _Playable.Play();
                else
                    _Playable.Pause();
            }
            //在改变运行状态（播放变暂停，或暂停变播放）后，额外执行的逻辑。
            OnSetIsPlaying();
        }
        ...
        /// <summary>Creates and assigns the <see cref="Playable"/> managed by this state.
        /// <para>创建Playable节点，并且连接到父节点上（其实就是AnimationMixerPlayable节点）</para>
        /// </summary>
        /// <remarks>This method also applies the <see cref="AnimancerNodeBase.Speed"/> and <see cref="IsPlaying"/>.</remarks>
        protected sealed override void CreatePlayable() //sealed就是不再允许派生类重写该方法了
        {
        ...
        /// indicate active.
        /// </remarks>
        internal int _ActiveIndex = ActiveList.NotInList; //固定就是-1，但是这样设置一个const常量确实更加直观，增强逻辑性，对于一个好的代码不可或缺
        ...
        /// </remarks>
        public bool IsActive //Tip：也可以以封装的思想来看待该属性，就是直接表明是否对于输出结果有影响
            => _ActiveIndex >= 0;
        ...
        internal void UpdateIsActive()
        {
            ...
            var parent = Parent;
            if (parent != null)
                parent.ApplyChildActive(this, shouldBeActive); //对于parent为AnimancerLayer，就是ActiveStatesInternal的Add和Remove
            else
