...
namespace Animancer
{
        ...
        ICopyable<AnimancerLayer>
    {
        ...
        /// <summary>[Internal] Creates a new <see cref="AnimancerLayer"/>.
        /// <para>创建AnimancerLayer实例以及对应的AnimationMixerPlayable节点</para>
        /// </summary>
        ...
        /// <summary>The animation states connected to this layer.
        /// <para>记录连接到该Layer的状态节点</para>
        /// </summary>
        ...
        protected internal override void OnRemoveChild(AnimancerState state)
        {
            ...
            //最直观的就是这里的断开连接。
            if (Graph._PlayableGraph.IsValid() && _Playable.GetInput(index).IsValid())
        ...
        /// <summary>Returns a state registered with the `key` and attached to this layer or null if none exist.
        /// <para>首先尝试从Key进行类型转换获取AnimancerState，然后尝试从AnimancerGraph的AnimancerStateDictionary字典中尝试获取该键对应的实例
        /// （其实一般情况下，该字典中的键就是AnimancerState，与其值是同一个）。</para>
        /// </summary>
        ...
        /// <para>获取或者创建对应key的AnimancerState（包括Playable节点以及连线）</para>
        /// </summary>
        public AnimancerState GetOrCreateState(ITransition transition)
        {
            /*Tip：这里尤其要注意，假如传入的是TransitionAsset，则这里的transition.Key要看它的检视面板中Transition字段是什么类型的Transition，
            也可以参考TransitionAsset的Reset方法（默认为ClipTransition），总之对于Key属性的具体实现要看这些具体的运行时Transition类，否则就只能看到个
            无实现的接口，这也算是多态性带来的一种迷惑性。*/
            var key = transition.Key;
            var state = GetState(ref key); //尝试直接获取AnimancerState
            //没有现成就创建
            if (state == null)
            {
                //Tip：这里也要看具体的运行时Transition类型（如ClipTransition，LinearMixerTransition）中实现的CreateState方法
                state = transition.CreateState(); //此时只是创建AnimancerState实例，还没有创建Playable相关内容
                state._Key = key;
                state.SetParent(this); //创建和处理PlayableGraph相关内容。
            }
        ...
        public AnimancerState GetOrCreateState(AnimancerState state)
        {
            ...
            if (parent == null) //独立节点直接设置即可，不用考虑对之前parent的善后工作。
            {
                state.SetParent(this);
                return state;
            }

            //在state的parent不是该Layer时，就需要执行以下的一系列逻辑了，否则就像上面一样直接返回其本身即可，

            var key = state.Key;
            key ??= state; //没有key就直接将其自身作为key，但是这里完全依赖于IAnimancerComponent的GetKey方法是如何实现的

            var stateOnThisLayer = GetState(ref key);
            //Tip：在当前Layer中实在没找到指定的State，就新建一个，并且是利用了对象池以及克隆技巧，值得学习
        ...
        public AnimancerState Play(AnimancerState state)
        {
            ...
            //Tip：注意这个FadeGroup==null，对于Animancer如何实现动画过渡有所提示。
            if (Weight == 0 && FadeGroup == null)
                Weight = 1;

            CurrentState?.FadeGroup?.Cancel(); //打断过渡
            /*Tip：注意与AnimancerStateDictionary的GetOrCreate方法区别，在那个方法中是根据传入的AnimationClip创建用于封装的ClipState实例，然后注册进入AnimancerStateDictionary，
            ...
            for (int i = ActiveStatesInternal.Count - 1; i >= 0; i--)
            {
                ...
                //通过设置权重为0，即去除影响，来实现停止的效果。不过还要注意重写的StopWithoutWeight方法
                if (otherState != state)
                    otherState.Stop(); 
            }
        ...
        public AnimancerState Play(AnimancerState state, float fadeDuration, FadeMode mode = default)
        {
            ...
            //Tip：注意这是AnimancerLayer，对应AnimationMixerPlayable，但实际会发现是连接在该节点的输入端口上的AnimationClipPlayable的输入权重在变化，所以实际的过渡逻辑更加复杂。
            //Ques：暂时不明白这里为何要调用StartFade方法，因为真正实现子节点过渡的是下面的SetNodes部分的代码，大概是可以实现在多个Layer之间的过渡？而不只是AnimationClipPlayable之间的过渡。
            StartFade(1, layerFadeDuration); //AnimancerNode的方法
            ...
            state = GetOrCreateState(state);
            //可见刚开始过渡时就会将CurrentState设置为过渡目标。这一点应该会涉及到一些机制
            CurrentState = state;
        ...
        private static bool IsAlreadyFadingIn( AnimancerState state, float fadeDuration)
        {
            if (!state.IsPlaying)
                return false;
        ...
        public AnimancerState Play(ITransition transition)
            ...
            ? Graph.Transitions.Play(this, transition) //纯粹是为了到
        ...
        {
            var state = GetOrCreateState(transition); //状态实例与Playable节点和连线。
            state = Play(state, fadeDuration, mode); //这里就已经开始过渡了，下面的Apply方法只是添加一些额外逻辑，典型的就是设置转换开始的进度位置。
            transition.Apply(state); //对于ClipTransition，就是设置Start Time转换从片段的指定时刻开始。
            return state;
        }
        ...
        /// </exception>
        private void EvaluateFadeMode( FadeMode mode, ref AnimancerState state, float fadeDuration, out float stateFadeSpeed, out float layerFadeDuration)
        {
        ...
        /*Tip：从PlayableGraph的运行来看，其实将AnimancerLayer对应的节点（AnimationMixerPlayable）的输出权重设置为0之后，其子节点的输出就没有影响了，
        而在此处进行清理，一方面是为了保证记录的状态符合当前实际，一方面是可以减少无用的计算，节省性能。*/
        /// <summary>
