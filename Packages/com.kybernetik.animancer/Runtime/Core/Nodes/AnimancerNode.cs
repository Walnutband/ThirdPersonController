...
namespace Animancer
{
        ...
        IHasDescription
    {
        ...
        private void SetWeightInternal(float value)
        {
            ...
            _Weight = value;
            //Tip: 都是通过父节点调用SetInputWeight设置权重，传入自己就是提供自己应该的权重的信息。
            if (Graph != null)
        ...
        public FadeGroup FadeGroup
        {
            ...
            internal set
            {
                _FadeGroup?.Remove(this); //通知当前所在的FadeGroup
                _FadeGroup = value;
            }
        }
        ...
        public void StartFade(float targetWeight, float fadeDuration)
        {
            ...
            else if (fadeDuration > 0)
            {
                ...
                //从对象池中请求一个FadeGroup实例
                var fade = FadeGroup.Pool.Instance.Acquire();
                fade.SetFadeIn(this); //过渡进入该节点（在UI中会把FadeIn说成“渐入“，确实直观，但是似乎没有Transition的含义，虽然也区别不大）
                fade.StartFade(targetWeight, fadeSpeed); //设置FadeGroup的相关属性值，以及注册进入Update容器
            }
            else
            {
                Weight = targetWeight;
        ...
        /// <summary>[Internal] Stops this node without setting its <see cref="Weight"/>.
        /// </summary>
        /// <remarks>重写该方法实现不设置权重来停止节点的方式</remarks>
