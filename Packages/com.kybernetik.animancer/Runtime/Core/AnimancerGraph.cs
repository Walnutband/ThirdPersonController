...
namespace Animancer
{
        ...
        IHasDescription
    {
        ...
        /// <summary>The <see cref="AnimancerState"/>s managed by this graph.
        /// <para>维护该Graph中的State（AnimancerState）与Clip（AnimationClip）的映射关系</para>
        /// </summary>
        /// <remarks>
        /// <strong>Documentation:</strong>
        ...
        /// <remarks>用于对在该Graph中进行的状态转换（动画过渡）进行覆盖</remarks>
        ...
        //TODO：命名不合适，感觉应该叫Executor执行器，Controller控制器之类的，或者就叫Director其实非常贴切。
        /// <summary>The component that is playing this <see cref="AnimancerGraph"/>.
        /// <para>正在播放该AnimancerGraph的Animancer组件，其实就相当于控制PlayableGraph的PlayableDirector组件</para>
        /// </summary>
            ...
            TransitionLibrary transitions = null)
        {
            ...
            States = new(this); //位于该Graph中的管理状态映射表的字典。
        ...
        /// <para></para>创建空的PlayableGraph，并命名。
        /// </summary>
        /// <remarks>
...
#else 
            //在打包之后命名就毫无意义了，因为只是用于编辑器中的编辑窗口查看的。
        ...
        /*TODO：将该方法公开被其他调用，也就是被那些会创建AnimancerGraph的地方所调用，传入与其相关的名称，作为该AnimancerGraph所封装的PlayableGraph的名称。
        但是很别扭的一点是，在调用该方法后都会调用AnimancerGraph的构造方法，其实也可以将这个name作为构造方法的一个参数，不用分开，因为本来就是强绑定，要么
        创建空的PlayableGraph，要么就是传入已经存在的PlayableGraph。*/
        ...
        {
            //Tip：这就是创建AnimancerGraph时的异常处理代码，典型的商用级插件才会专门编写的内容，不过对于学习者来说其实不用过于关注，因为其并不直接关系到核心功能。
#if UNITY_ASSERTIONS
            //没有Animator组件
            if (animator == null)
...
#endif
            //创建AnimationLayerMixerPlayable节点（就是对应于Animator状态机中的Layers列表，连接AnimationMixerPlayable节点就相当于增加新的Layer层级）
            //注意这里的AnimancerLayerMixerList，代表的是在当前Graph中的各个层级的容器，调用其SetMask传入AvatarMask就可以实现动画遮罩的效果。
            Layers ??= new AnimancerLayerMixerList(this);

            Component = animancer; //控制者
            ...
            //Tip：初始化事件相关的单例类，专门用于实现动画系统的事件功能，其实也是对AnimationEvent的封装，不过没有使用内置的编辑方式，而是额外的一套自定义的编辑方式。
...
#pragma warning disable CS0618 // Type or member is obsolete.
            ...
            /*TODO：这里Play已经显示被弃用，因为会覆盖PlayableGraph的TimeUpdateMode，但是也没有给出明确的替代方案，不过这本来就只是一个快捷方式，所以也完全可以使用其他Playable的API实现同样功能。
            */
            /*还没写完
            var output = AnimationPlayableOutput.Create(_PlayableGraph, "AnimationPlayableOutput", animator);
            _PlayableGraph.Connect(_PreUpdatePlayable, 0, output, 0);
            _PlayableGraph.Play();
            */

            /*Tip：
            方法功能：
            该方法在指定的 Animator 上播放给定的 Playable，内部等价于：创建一个 AnimationPlayableOutput，将传入的 Playable 绑定到输出端口 0，设置 PlayableGraph 的时间更新模式为 GameTime，并调用 Play() 启动播放。
            
            对 PlayableGraph 节点结构的影响：
            添加一个 AnimationPlayableOutput 节点，用于驱动 Animator
            通过 SetSourcePlayable(playable, 0) 将传入的 Playable 接入该输出节点的输入端口 0
            覆盖并设置 PlayableGraph 的 Time Update Mode（默认改为 DirectorUpdateMode.GameTime）
            最终调用 graph.Play() 使整个图开始运行
            上述操作不会修改已有 Playable 节点之间的连接或拓扑，只是向图中附加了新的输出节点并启动播放。
            */
            AnimationPlayableUtilities.Play(animator, _PreUpdatePlayable, _PlayableGraph);
#pragma warning restore CS0618 // Type or member is obsolete.
            //这里只是标记，上面的Play方法才是真正播放PlayableGraph的逻辑。而播放Graph的本质是让Graph进入播放状态，实际播放应该是在下一帧中开始，那么下一帧就是基于这一帧添加在Graph中的节点。
        ...
        private void DisposeAll()
        {
            ...
            /*Tip：活久见，竟然看到了goto语句，但是在编程中似乎完全不鼓励使用goto语句，因为很容易破坏代码结构，而且引入潜在风险，虽然可以在某些非常特殊的情况下使用来简化代码，
            但是终究有可以完全替代的其他写法，在这少数情况引入goto语句就有些破坏和谐性了*/
            DisposeNext:
            try
            {
                while (--i >= 0)
        ...
        public object GetKey(AnimationClip clip)
            => Component != null
            ? Component.GetKey(clip) //说明如果自定义Animancer组件的话，就是重写GetKey方法自定义如果根据AnimationClip生成一个Key。
            : clip; //否则直接将Clip本身作为Key，不过自带的AnimancerComponent其实就是直接返回Clip本身
        ...
        /// <summary>The graph currently being executed.正在被执行的Graph</summary>
        /// <remarks>
        ...
        {
            //Tip：因为在以下的逻辑中会用到Current，所以需要临时设置为此时执行该方法（UpdateAll）的AnimancerGraph实例，执行完成之后就恢复（前提是这里单线程，如果是多线程呢？）
            var previous = Current;
            Current = this;
