...
namespace UnityEditor.Timeline
{
    ...
    {//在这里规定了Playback的所有控制，就是五个按钮（中间按钮兼任Play和Pause）加上鼠标直接点击和拖拽时间线PlayHead。
        void Play();
        void Pause();
        void PreviousFrame();
        void NextFrame();
        void GoToFirstFrame();
        void GoToLastFrame();
        //在鼠标点击和拖拽PlayHead时就是调用以下方法来执行逻辑，double和int分别对应以时间为单位和以帧为单位。
    ...
    /// <summary>
    /// Use the TimelinePlaybackControls to manage the Timeline window's playback state, playhead location（时间线位置）, and play range.
    /// </summary>
