...
namespace UnityEditor.Timeline
{
    partial class TimelineWindow
    {
        ...
        public Rect sequenceContentRect
        {
            get
            {
                return new Rect(
                    ...
                    WindowConstants.markerRowYPosition, //别忘了在时间轴下方还有个marker区域，再下面才是Track和Clip的区域（虽然Marker也是一个Track，但是区别对待）。
                    //整个窗口宽度，减去左半边宽度，再减去右边界处的数值滑动条宽度。
        ...
        void DoLayout()
        {
            ...
            Initialize();
            //只要调用OnGUI，就必然会至少有Layout和Repaint事件。
