...
namespace UnityEngine.Timeline
{
    ...
    { //注意Track包含普通Track以及Group Track，但是实际使用的只有普通Track
        ...
        [SerializeField, HideInInspector] bool m_Locked; //Tip：用于编辑时
        ...
        internal Playable CreatePlayableGraph(PlayableGraph graph, GameObject go, IntervalTree<RuntimeElement> tree, Playable timelinePlayable)
        {
            ...
            /*为当前轨道的所有 Marker/Signal 注册时点通知
            将上一步的 mixerPlayable 作为输入，包装在一个 ScriptPlayable<TimeNotificationBehaviour> 中
        ...
        internal virtual Playable CreateMixerPlayableGraph(PlayableGraph graph, GameObject go, IntervalTree<RuntimeElement> tree)
        {
            ...
            //TrackAsset派生类，实现ILayerable接口就可以支持层级，AnimatorController就是如此。
