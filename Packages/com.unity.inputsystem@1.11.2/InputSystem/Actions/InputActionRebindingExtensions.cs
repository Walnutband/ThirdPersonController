...
namespace UnityEngine.InputSystem
{
    ...
    public static partial class InputActionRebindingExtensions
    {
        ...
        public static RebindingOperation PerformInteractiveRebinding(this InputAction action, int bindingIndex = -1)
        {
            ...
            var rebind = new RebindingOperation() //链式调用设置各项内容
                .WithAction(action) //在该Action上执行重绑定操作。指定了Action就可以得知Control Type，就可以确定属性expectedControlType
                ...
                // 因为鼠标或者触摸屏的移动和所在位置通常用于UI操作，不应该被识别为重绑定按键。
            ...
            if (rebind.expectedControlType != "Button") //如果是按钮的话，感觉也不太可能会重绑定Escape键。
                rebind.WithCancelingThrough("<Keyboard>/escape");  //Escape取消重绑定
            ...
            if (bindingIndex >= 0)
            {
                ...
                rebind.WithTargetBinding(bindingIndex); //重绑定到指定索引上。
            }
