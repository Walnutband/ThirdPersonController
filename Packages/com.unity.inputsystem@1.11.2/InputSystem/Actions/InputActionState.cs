...
namespace UnityEngine.InputSystem
{
    ...
    internal unsafe class InputActionState : IInputStateChangeMonitor, ICloneable, IDisposable
    {
        ...
        private void EnableControls(InputAction action)
        {
            ...
            /*Tip：因为一个Map中的Binding是统一管理的，并没有像编辑窗口中显示的那样被InputAction分隔。所以这里就是遍历整个Map中的所有Binding，进行条件判断，
            筛选出符合条件的binding，然后将其绑定的控件注册到输入轮询容器中。*/
            ...
            for (var i = 0; i < bindingCount; ++i)
            {
                ...
                var controlCount = bindingState->controlCount; //没有绑定具体控件的Binding
                if (controlCount == 0)
                    continue;
        ...
        /// 这里的EnableControls方法揭示了启用一个InputAction的核心逻辑。
        ...
        private void EnableControls(int mapIndex, int controlStartIndex, int numControls)
        {
            ...
            for (var i = 0; i < numControls; ++i)
            {
                ...
                if (IsControlEnabled(controlIndex))
                    continue;
                //Tip：显然，注册的时候包含了control、binding和map的信息，并不是通过各自类中的成员来引用，而是通过这里统一的index索引映射关系来获取彼此。
        ...
        {
            //Ques：这里其实只用到了（从低到高）6个字节，但是long本身有8个字节。注意结合上面的ToCombinedMapAndControlAndBindingIndex来思考。
            controlIndex = (int)(mapControlAndBindingIndex & 0x00ffffff); //保留从低到高的24位，所以下面会右移24位
            bindingIndex = (int)((mapControlAndBindingIndex >> 24) & 0xffff); //除开controlIndex的24位，保留16位。
            mapIndex = (int)((mapControlAndBindingIndex >> 40) & 0xff); //除开control和binding，使用8位
        }
