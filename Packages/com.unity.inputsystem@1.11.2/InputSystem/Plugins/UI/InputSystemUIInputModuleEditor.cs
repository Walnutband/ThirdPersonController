...
namespace UnityEngine.InputSystem.UI.Editor
{
    ...
    internal class InputSystemUIInputModuleEditor : UnityEditor.Editor
    {
        ...
        /*Tip:获取指定的InputActionAsset资产中的所有InputActionReference资产
        原理在于，Asset和Reference资产是存储在同一个资产文件中的，Asset是主资产，Reference则是一个个子资产，
        通过以下的Linq方法，也就是通过Asset先找到其文件所在路径，然后加载该路径下、也就是该文件中的所有资产，包括主资产和若干子资产，
        得到其IEnumerable，然后将Reference资产全部取出来，转换为实际类型InputActionReference，按照名字排序（英文字母先后顺序），然后是
        转换为数组类型*/
        private static InputActionReference[] GetAllAssetReferencesFromAssetDatabase(InputActionAsset actions)
        {
            if (actions == null)
                return null;
            //编辑模式就适合使用AssetDatabase。
