...
namespace Unity.Cinemachine
{
    ...
    public sealed class CinemachineCamera : CinemachineVirtualCameraBase
    {
        ...
        public CameraTarget Target; //跟踪与瞄准的目标
        ...
        {
            /*Tip：这一步纯粹是体现逻辑性————每个CinemachineCamera都有一个组件管线，阶段都是由CinemachineCore.Stage所定义，并且在每个阶段只能有一个组件（CinemachineComponentBase）产生作用，
            所以这里的管线组件容器m_Pipeline的长度就等于阶段的个数，由于从0开始，所以作为最后阶段的Finalize需要+1才等于阶段的总个数。*/
