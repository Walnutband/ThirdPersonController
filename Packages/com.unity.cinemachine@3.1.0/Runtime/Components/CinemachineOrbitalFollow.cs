...
namespace Unity.Cinemachine
{
        ...
        , CinemachineFreeLookModifier.IModifiableDistance
    {
        ...
        {
            /*Tip：这里的命名，可以看CinemachineInputAxisController的ControllerManager的三个Controllers，不过其实就是从这里创建的，命名为此处的Name。
            还有这里分为了X轴和Y轴来分别获取输入，所以就能够处理Vector2类型的输入值。
            这里的DrivenAxis就是将该组件内的三个InputAxis即HorizontalAxis、VerticalAxis、RadialAxis*/
            ...
            /*BugFix: 这里存在一个Bug，由于RadialAxis就是实现镜头放缩功能的，所以通常绑定的是鼠标滚轮，而鼠标滚轮在输入值类型分类上似乎是Vector2，如果这里的Hint设置为默认的话，
            会导致在CinemachineInputAxisController.Reader中的ReadInput方法中是按照ReadValue<float>来读取，这样一来会导致类型不匹配，而返回默认值即default(float)=0.0f，所以
            这里要点名Y，但是感觉也不够准确，如果真的是使用float类型的InputAction的话，Hint就应该是Default。*/
