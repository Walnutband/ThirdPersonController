...
namespace Unity.Cinemachine
{
    ...
    internal class InputAxisControllerManager<T> where T : IInputAxisReader, new ()
    {
        ...
        {
            /*最直观的，给一个游戏对象挂载上CinemachineCamera、CinemachineOrbitalFollow和CinemachineInputAxisController，就可以在CinemachineInputAxisController的
            检视器中看到在CinemachineOrbitalFollow实现的GetInputAxes方法中指定的Name对应的控制器（类型就是下面的InputAxisControllerBase<T>.Controller）。*/
            OnDisable();
            //基于该控制器组件所在的游戏对象，获取所挂载的IInputAxisOwner组件
            if (scanRecursively)
            ...
            for (int j = 0; j < m_AxisOwners.Count; ++j)
            {
                ...
                //随着多次for循环，m_Axes会将各个IInputAxisOwner的AxisDescriptor都添加到该容器中，但是在下面这个循环中是从上一次的尾元素的下一个位置开始的，所以不会与前面的重复。
                for (int i = startIndex; i < m_Axes.Count; ++i)
                {
                    int controllerIndex = GetControllerIndex(Controllers, t, m_Axes[i].Name);
                    if (controllerIndex < 0) //还没有。
                    {
            ...
            Controllers = newControllers; //更新控制器列表，newControllers列表就是起到缓存作用。
