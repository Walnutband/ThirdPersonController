...
namespace Unity.Cinemachine
{
    ...
    public struct CameraState
    {
        ...
        /*Tip：这里命名略带迷惑，其实在CinemachineComponentBase派生的各个组件中的MutateCameraState方法中就是对传入的CameraState
        的RawPosition和RawOrientation进行了赋值更新，不过可以看到CinemachineBasicMultiChannelPerlin组件，也就是实现了Noise噪声，在它的MutateCameraState方法中
        就是对PositionCorrection和OrientationCorrection进行了赋值更新*/
        ...
        /// This value doesn't get fed back into the system when calculating the next frame.（这个值不会被传递到下一帧，这才是Correction修正值和Raw原始值的根本区别）
