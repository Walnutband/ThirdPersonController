...
namespace Unity.Cinemachine
{
        ...
        : InputAxisControllerBase<CinemachineInputAxisController.Reader> 
    {
        ...
        public sealed class Reader : IInputAxisReader
        {
            ...
            public InputActionReference InputAction; //直接在检视器中拖入指定的InputAction即可。
            ...
            public float Gain = 1; //放缩因子，通过正负实现反转功能
                ...
                IInputAxisOwner.AxisDescriptor.Hints hint)
            {
                ...
                if (InputAction != null)
                {
                    if (context is CinemachineInputAxisController c) //这是具体类型，也就是说该Reader专用于CinemachineInputAxisController
                    {
                ...
                IInputAxisOwner.AxisDescriptor.Hints hint)
            {
                ...
                if (m_CachedAction != null && m_CachedAction.enabled != InputAction.action.enabled)
                {
                    if (InputAction.action.enabled)
                        m_CachedAction.Enable(); //启用InputAction，才会开始接收输入
                    else
